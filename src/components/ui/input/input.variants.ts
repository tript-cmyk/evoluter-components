// Input styling variants placeholders

export const inputStatusStyles = {
  default: "border-[#333333] hover:border-[#444444]",
  active: "border-[#ABFFC3] shadow-[0_0_8px_rgba(171,255,195,0.15)]",
  success: "border-[#40A05B]",
  error: "border-[#FF5C5C] shadow-[0_0_8px_rgba(255,92,92,0.08)]",
  processing: "border-[#333333]",
  disabled: "border-[#2A2A2A] bg-[#1A1A1A] opacity-50 pointer-events-none cursor-not-allowed",
};
